...
import { db } from "./db";
import { eq, sql, and, gte, lte, inArray, lt } from "drizzle-orm";
...
export interface IStorage {
  ...
  updateIncidentType(
    id: string,
    data: Partial<InsertIncidentType>,
...
export class DatabaseStorage implements IStorage {
  ...
  async updateEmployee(
    ...
    if (employeeData.employeeNumber !== undefined)
      updateData.employeeNumber = employeeData.employeeNumber;
    ...
    if (employeeData.hireDate !== undefined)
      updateData.hireDate = employeeData.hireDate;
    ...
    const [dept] = updatedUser.departmentId
      ? await db
          .select()
          .from(departments)
          .where(eq(departments.id, updatedUser.departmentId))
          .limit(1)
      : [undefined];
  ...
  async getTimeEntries(): Promise<TimeEntry[]> {
    ...
    const entries = await Promise.all(
      workdays.map(async (workday) => {
        ...
        // Obtener pausas del día
        const breaks = await this.getBreaksForDay(
          workday.employeeId,
          workday.date,
        );
  ...
  async createTimeEntry(timeEntry: InsertTimeEntry): Promise<TimeEntry> {
    ...
    if (existingWorkday) {
      ...
      if (
        !existingWorkday.startTime ||
        timeEntry.clockIn < existingWorkday.startTime
      ) {
      ...
      if (timeEntry.clockOut) {
        if (
          !existingWorkday.endTime ||
          timeEntry.clockOut > existingWorkday.endTime
        ) {
        ...
        if (existingWorkday.startTime && timeEntry.clockOut) {
          const startMs = (
            existingWorkday.startTime || timeEntry.clockIn
          ).getTime();
      ...
      const workedMinutes = timeEntry.clockOut
        ? Math.floor(
            (timeEntry.clockOut.getTime() - timeEntry.clockIn.getTime()) /
              60000,
          )
        : 0;
  ...
  async updateTimeEntry(
    id: string,
    timeEntryData: Partial<InsertTimeEntry>,
  ): Promise<TimeEntry | undefined> {
    ...
    const endTime =
      timeEntryData.clockOut !== undefined
        ? timeEntryData.clockOut
        : existing.endTime;
    ...
    const totalMinutes =
      updatedWorkday.workedMinutes - updatedWorkday.breakMinutes;
    const totalHours = totalMinutes / 60;

    // Obtener pausas del día
    const breaks = await this.getBreaksForDay(
      updatedWorkday.employeeId,
      updatedWorkday.date,
    );
  ...
  async createIncident(insertIncident: InsertIncident): Promise<Incident> {
    ...
    let workday = await this.getDailyWorkdayByEmployeeAndDate(
      insertIncident.userId,
      insertIncident.date,
    );
  ...
  async updateIncident(
    ...
    if (incidentData.userId !== undefined)
      updateData.userId = incidentData.userId;
    ...
    if (incidentData.incidentType !== undefined)
      updateData.incidentType = incidentData.incidentType;
  ...
  async createDateSchedule(
    ...
    const workHours =
      (insertDateSchedule as any).workHours ??
    ...
    const [shift] = await db
      ...
      .values({
        ...
        shiftType: shiftType,
        status:
          insertDateSchedule.isActive === false ? "cancelled" : "scheduled",
      })
      .returning();
  ...
  async updateDateSchedule(
    ...
    if (dateScheduleData.startTime !== undefined)
      updateData.expectedStartTime = dateScheduleData.startTime;
    if (dateScheduleData.endTime !== undefined)
      updateData.expectedEndTime = dateScheduleData.endTime;
    ...
    const [startHour, startMin] = updatedShift.expectedStartTime
      .split(":")
      .map(Number);
    const [endHour, endMin] = updatedShift.expectedEndTime
      .split(":")
      .map(Number);
    const startMinutes = startHour * 60 + startMin;
    const endMinutes = endHour * 60 + endMin;
    ...
    return {
      ...
      workHours: workMinutes,
      isActive:
        updatedShift.status === "scheduled" ||
        updatedShift.status === "confirmed" ||
  ...
  async createManualDailyWorkday(data: {
    ...
    if (entries.length > 0) {
      throw new Error(
        "No se puede crear una jornada manual porque ya existen fichajes automáticos para este día",
      );
    }
    ...
    if (existingWorkday) {
      throw new Error(
        "Ya existe una jornada laboral para este empleado en esta fecha",
      );
    }
  ...
  async updateManualDailyWorkday(
    ...
    if (entries.length > 0) {
      throw new Error(
        "No se puede editar una jornada porque ya existen fichajes automáticos para este día",
      );
    }
  ...
  async deleteDailyWorkday(id: string): Promise<boolean> {
    ...
    if (entries.length > 0) {
      throw new Error(
        "No se puede eliminar una jornada porque ya existen fichajes automáticos para este día",
      );
    }
  ...
  async crearFichaje(
    ...
    if (entryType === "clock_in") {
      ...
      if (yaHayClockIn) {
        throw new Error(
          "Ya has iniciado jornada hoy. No puedes iniciar jornada dos veces el mismo día.",
        );
      }
      ...
      if (jornadaHoy && jornadaHoy.status === "closed") {
        throw new Error(
          "Ya has finalizado tu jornada hoy. No puedes iniciar otra jornada el mismo día.",
        );
      }
    }
    ...
    if (entryType === "clock_out") {
      ...
      if (!hayClockIn) {
        throw new Error(
          "No has iniciado jornada hoy. Debes iniciar jornada antes de finalizar.",
        );
      }
      ...
      const breakStarts = fichajesHoy.filter(
        (f) => f.entryType === "break_start",
      );
      const breakEnds = fichajesHoy.filter((f) => f.entryType === "break_end");
      if (breakStarts.length > breakEnds.length) {
        throw new Error(
          "No puedes fichar salida mientras tienes una pausa activa. Finaliza la pausa primero.",
        );
      }
    }
    ...
    if (entryType === "break_start") {
      ...
      if (!hayClockIn) {
        throw new Error(
          "No has iniciado jornada hoy. Debes fichar entrada antes de iniciar una pausa.",
        );
      }

      if (hayClockOut) {
        throw new Error(
          "Ya has finalizado tu jornada hoy. No puedes iniciar una pausa.",
        );
      }
    ...
    if (entryType === "break_end") {
      ...
      if (breakStarts.length === 0) {
        throw new Error(
          "No has iniciado ninguna pausa hoy. Inicia una pausa antes de finalizarla.",
        );
      }
 ...
 */
async function calcularYActualizarJornada(
  employeeId: string,
  fecha: string,
): Promise<void> {
      ...
      case "clock_out":
        ...
        if (lastClockIn) {
          const minutes = Math.floor(
            (endTime.getTime() - lastClockIn.getTime()) / 60000,
          );
...
// Función para calcular el número de empleados actuales vs semana pasada
export async function getEmployeeGrowth() {
  // Fechas de referencia
  const now = new Date();
  const startOfThisWeek = new Date(now);
  startOfThisWeek.setDate(now.getDate() - now.getDay()); // lunes actual
  ...
  // Contar empleados activos contratados hasta el final de cada semana
  const [{ count: currentCount }] = await db
    .select({ count: sql<number>`count(*)` })
    .from(users)
    .where(and(eq(users.isActive, true), lt(users.hireDate, now)));

  const [{ count: lastWeekCount }] = await db
    .select({ count: sql<number>`count(*)` })
    .from(users)
    .where(and(eq(users.isActive, true), lt(users.hireDate, endOfLastWeek)));

  // Calcular diferencia
  const difference = currentCount - lastWeekCount;
  const percentageChange =
    lastWeekCount > 0
      ? ((difference / lastWeekCount) * 100).toFixed(2)
      : "100.00";
